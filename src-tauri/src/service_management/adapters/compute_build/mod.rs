...
pub async fn list_resources(
    ...
    match service_key {
        "lambda" => lambda::list_resources(config).await,
        "ec2" => ec2::list_resources(config).await,
        "ecs" => ecs::list_resources(config).await,
        "eks" => eks::list_resources(config).await,
        "ecr" => ecr::list_resources(config).await,
        "msk" => msk::list_resources(config).await,
        "codebuild" => codebuild::list_resources(config).await,
        "codedeploy" => codedeploy::list_resources(config).await,
        "autoscaling" => autoscaling::list_resources(config).await,
        "bedrockruntime" => bedrockruntime::list_resources(config).await,
...
pub async fn execute_action(
    ...
    match request.service_key.as_str() {
        "lambda" => lambda::execute_action(config, request).await,
        "ec2" => ec2::execute_action(config, request).await,
        "ecs" => ecs::execute_action(config, request).await,
        "ecr" => ecr::execute_action(config, request).await,
        "codebuild" => codebuild::execute_action(config, request).await,
        "codedeploy" => codedeploy::execute_action(config, request).await,
        "autoscaling" => autoscaling::execute_action(config, request).await,
        "bedrockruntime" => bedrockruntime::execute_action(config, request).await,
        service_key
...
mod tests {
    ...
    #[tokio::test]
    async fn read_only_compute_services_allow_inventory_refresh_action() {
        let request = ServiceActionRequest {
            service_key: "eks".to_owned(),
        ...
        assert_eq!(result.message, "Refreshed `eks` inventory.");
    }
